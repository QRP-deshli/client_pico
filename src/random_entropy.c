/*
Program taken from Pico SDK rand folder. 
Initially must be used for random number generation, but using 
xoroshiro prng, which is not cryptographically secure, I've modified 
this project for entropy generation only, function names and other macros 
wasnt changed.
Three major modifications:
# Already setted up entropy sourses: Timestamp, ROSC and RAM hash.
# Deleted function for generation of 32 bytes chunks.
# Changed logic at the end of get_rand_64, its not using xoroshiro now, 
instead it xoring samples from different entropy sourses to a 64 bytes chunk 
at the end.
*/
...
#include "include/random_entropy.h"
...
uint64_t get_rand_64(void) {
    ...
    //#NK we use this function for seed generation, so no need to use additional
    //prng, we just need to extract entropy
    //uint64_t rand64 = xoroshiro128ss(&local_rng_state);
    
    uint64_t rand64;
    rand64 = local_rng_state.r[0] ^ local_rng_state.r[1];
    rng_state = local_rng_state;
    check_byte++;
    spin_unlock(lock, save);
